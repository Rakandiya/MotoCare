...
export default function Katalog() {
    ...
    const handleMerkSearch = (e) => {
        ...
        if (e.target.value) {
            ...
            const filteredMerks = [...product.keys()].filter((merk) =>
                merk.includes(searchValue)
            );
    ...
    const handleLineupSearch = (e) => {
        ...
        if (LineupSearchValue) {
            ...
            setLineupList(product.get(currentBrand));
        }
    };
    ...
    const handleBrandClick = (merk) => {
        setCurrentBrand(merk);
        setCurrentProduct(null);
        setLineupList(product.get(merk));
    };
    ...
    return (
        <UserLayout>
            <h1>halo world</h1>
            {/* <div className="backgroundImage" id="bambang">
      <main>
        <article>
          <section>
            <div className="container mt-4">
              ...
              <section className={styles["form-katalog"]}>
                          ...
                          <div id="lineup-list">
                            {lineupList && lineupList.map((lineup) => (
                              <div
                                ...
                                id={lineup}
                                className={styles["merk"]}
                                onClick={() => handleProductClick(lineup)}
                                style={{ cursor: 'pointer' }}
                              >
                                {lineup}
                              </div>
                ...
                <div className={styles["right-panel"]} id="Konten">
                  {currentProduct && <img title="Gambar" id="gambar-produk" src={detailproduct.get(currentProduct)[0]} className="motor-image"/> }
                  <div className={styles["title-deskripsi"]} id="title">
                    {currentProduct && detailproduct.get(currentProduct)[1]}
                  </div>
                  <p className={styles["description"]} id="description">
                    {currentProduct && detailproduct.get(currentProduct)[2]}
                  </p>
                </div>
